...
GraphicWindow::GraphicWindow(int width, int height, const char *title, Color backgroundColor, Color cAxes)
...
void GraphicWindow::update(double deltaTime) {
...
void GraphicWindow::addFunction(Function &func, Color color) {
    _funcs.emplace_back(func, getColor(color));
}

void GraphicWindow::setView(double _xOffset, double _yOffset) {
    _xOffset = _xOffset;
    _yOffset = _yOffset;
}

void GraphicWindow::dragging() {
    static int prevX;
    static int prevY;
...
bool GraphicWindow::onScreen(int cX, int cY) const {
    return 0 <= cX && cX < width && 0 <= cY && cY < height;
}

double GraphicWindow::toMathX(int x) const {
    return (x - width / 2) / _scale + _xOffset;
}

double GraphicWindow::toMathY(int y) const {
    return -(y - height / 2) / _scale + _yOffset;
}

int GraphicWindow::toCrdX(double x) const {
    return (x - _xOffset) * _scale + width / 2;
}

int GraphicWindow::toCrdY(double y) const {
    return -(y - _yOffset) * _scale + height / 2;
}

void GraphicWindow::drawAxes() {
    ...
    x0 = std::min(x0, width - 1 - markupLength);
    Utilities::drawLine(window, x0, 0, x0, height - 1, _axisColor);
    double stY = Utilities::getCloseLowerNum(toMathY(height - 1), _axesStep);
    for (double curY = stY; curY <= toMathY(0); curY += _axesStep) {
        if (curY) {
            Utilities::drawLine(window, x0 - markupLength, toCrdY(curY), x0 + markupLength, toCrdY(curY), _axisColor);
    ...
    y0 = std::min(y0, height - 1 - markupLength);
    Utilities::drawLine(window, 0, y0, width - 1, y0, _axisColor);
    double stX = Utilities::getCloseLowerNum(toMathX(0), _axesStep);
    for (double curX = stX; curX < toMathX(width - 1); curX += _axesStep) {
        if (curX) {
            Utilities::drawLine(window, toCrdX(curX), y0 - markupLength, toCrdX(curX), y0 + markupLength, _axisColor);
...
void GraphicWindow::construct(Function &func, sf::Color color) {
    bool isArgX = func.getArgument() == "x";
    for (int cArg = 0; cArg < (isArgX ? width : height); cArg++) {
        ...
        cRes2 = std::min(cRes2, isArgX ? height - 1 : width - 1);
        Utilities::drawLine(window, isArgX ? cArg : cRes1, isArgX ? cRes1 : cArg, isArgX ? cArg : cRes2,
                            isArgX ? cRes2 : cArg, color);
    }
}

void GraphicWindow::graphMark() {
    ...
    bool isOnScreen = 0 <= cDotX + dotRadius && cDotX - dotRadius < width && 0 <= cDotY + dotRadius &&
                      cDotY - dotRadius < height;
